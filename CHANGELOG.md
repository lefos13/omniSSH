...
#### 2. Password File Import
* **Password File Format**: OmniSSH supports plaintext password files containing `user@host = password` entries (one per line, with `#` and `;` comment lines), allowing bulk credential imports for saved hosts without manual password entry.
* **Dashboard Import Any Time**: A new "Import Passwords" action on the Hosts dashboard toolbar allows importing passwords for hosts from any source (OpenSSH, Termius, manual entry) at any time. When no saved hosts exist yet, an empty state guides the user to add or import hosts first.
* **Shortcut From MobaXterm Import**: Finishing a MobaXterm session import offers this same flow directly as an optional second step, matching MobaXterm's native "Stored Passwords" export format so imported sessions can be populated immediately. The "add its password before connecting" reminder is hidden once at least one password is saved.
* **Match Preview Before Anything Is Written**: Every saved host whose username and host match an entry is listed with its configured storage and whether it would replace an existing password. Host matching ignores case and covers all saved hosts on any port, so one entry can match multiple hosts. Key-login hosts are excluded, and any row can be unticked so existing passwords are never replaced unexpectedly.
* **Overwrite Confirmation**: Rows that would replace an existing saved password are flagged, and Save stays disabled until the replacement is acknowledged. The summary afterwards reports what was actually stored per storage backend, names each host that failed, and suggests deleting the plaintext password file now that credentials are saved.
* **Per-Host Storage Routing**: Each password lands where that host's credential storage already points — encrypted App Vault ciphertext or the System Keychain. The App Vault unlock dialog only appears when a selected host targets the vault, and a vault that is locked or fails mid-run reports those hosts as failures rather than quietly falling back to the System Keychain; keychain hosts in the same run still succeed.
* **Duplicate Conflicts Are Refused**: An entry defined multiple times with conflicting passwords is counted as a conflict and written nowhere, and unmatched or malformed lines are reported in preview counts.
* **Secrets Stay In Rust**: The password file is read, parsed, and committed entirely in the Rust backend. Passwords never reach the frontend, the IPC payloads, or logs — preview and save results carry host IDs, labels, statuses, and counts only.
